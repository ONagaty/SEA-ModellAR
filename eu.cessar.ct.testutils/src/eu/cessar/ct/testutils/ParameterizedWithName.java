 ...
 *
 * <p>
 * Each instance of <code>FibonacciTest</code> will be constructed using the two-argument constructor and the data
 * values in the <code>&#064;Parameters</code> method.
 *
 * <p>
 * In order that you can easily identify the individual tests, you may provide a name for the
 * <code>&#064;Parameters</code> annotation. This name is allowed to contain placeholders, which are replaced at
 ...
 * <dd></dd>
 * </dl>
 * In the example given above, the <code>Parameterized</code> runner creates names like <code>[1: fib(3)=2]</code>. If
 * you don't use the name parameter, then the current parameter index is used as name.
 * </p>
 *
 * You can also write:
 *
 ...
 *
 * <p>
 * Each instance of <code>FibonacciTest</code> will be constructed with the default constructor and fields annotated by
 * <code>&#064;Parameter</code> will be initialized with the data values in the <code>&#064;Parameters</code> method.
 * </p>
 *
 * @since 4.0
 */
	...
	private FrameworkMethod getParametersMethod() throws Exception
	{
		...
		for (FrameworkMethod each: methods)
		{
			if (Modifier.isStatic(each.getMethod().getModifiers())
				&& Modifier.isPublic(each.getMethod().getModifiers()))
			{
				return each;
			}
		}
