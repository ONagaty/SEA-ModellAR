 ...
 *
 * File created by uidu0944 May 24, 2012 3:46:18 PM </copyright>
 */
...
/**
 * CessarJETReader is an input buffer for the JSP parser. It should allow unlimited lookahead and push-back. It also has
 * a bunch of parsing utility methods for understanding html-style things.
 */
@SuppressWarnings({"javadoc", "nls"})
public class CessarJETReader
{
	private static final List<String> JET_ATTRIBUTES = Arrays.asList("jet", "skeleton", "package", "imports", "class",
		"nlString", "startTag", "endTag", "extends", "implements", "methodname", "methodName", "throws", "parameters",
		"writer", "parallel", "include", "file");

	protected char startTagInitialChar = '<';
	protected char endTagInitialChar = '%';
	protected char endTagFinalChar = '>';
	protected CessarJETMark current;
	protected String master;
	protected List<String> sourceFiles = new ArrayList<>();
	protected List<String> baseURIs = new ArrayList<>();
	protected int size;
	protected boolean trimExtraNewLine = true;

	private StringBuilder curr = new StringBuilder();

	public CessarJETReader(String baseURI, String locationURI, InputStream inputStream, String encoding)
		throws JETException
	{
		stackStream(baseURI, locationURI, inputStream, encoding);
	}

	public CessarJETReader(String locationURI, InputStream inputStream, String encoding) throws JETException
	{
	 ...
	 *
	 * @param iStream
	 *        Stream ready to parse
	 * @param encoding
	 *        Optional encoding to read the file.
	 */
	public void stackStream(String baseURI, String locationURI, InputStream iStream, String oldEncoding)
		throws JETException
	{
		String encoding = oldEncoding;

		InputStreamReader reader = null;
		try
		{
			// Until the encoding can be specified within the template
			// we need to assume an encoding capable of working with any
			// character set.
			if (encoding == null)
			{
				encoding = "UTF8";
			...
			{
				// Remove zero width non-breaking space, which may be used as a
				// byte order marker,
			...
			if (current == null)
			{
				current = new CessarJETMark(this, writer.toCharArray(), fileid, locationURI, encoding);
			}
			else
			{
	...
	{
		// Is stack created ? (will happen if the JET file we're looking at is
		// missing.
		//
		if (current == null)
		{
	...
	/**
	 * Register a new baseURI. This method is used to implement file inclusion. Each included file gets a unique
	 * identifier (which is the index in the array of base URIs).
	 */
	...
	public int nextChar()
	{
		...
		curr.append(current.stream[current.cursor]);
	...
	/**
	 * Gets Content until the next potential JSP element. Because all elements begin with a '&lt;' we can just move
	...
	public char[] getChars(CessarJETMark start, CessarJETMark stop)
	{
		CessarJETMark oldstart = mark();
		reset(start);
		CharArrayWriter writer = new CharArrayWriter();
	...
	public CessarJETMark mark()
	{
		return new CessarJETMark(current);
	}

	public void reset(CessarJETMark mark)
	{
		current = new CessarJETMark(mark);
	}

	public boolean matchesIgnoreCase(String string)
	{
		CessarJETMark mark = mark();
		int ch = 0;
		int i = 0;
		do
		{
	 ...
	 * @param limit
	 *        The String to match.
	 * @return A non-null <code>CessarJETMark</code> instance if found, <strong>null</strong> otherwise.
	 */
	public CessarJETMark skipUntil(String limit)
	{
		CessarJETMark ret = null;
	 ...
	 * <code>skipSpaces</code>. The name is always considered case insensitive, hence stored in its lower case version.
	 *
	 * @param into
	 *        The HashMap instance to save the result to.
	 */
	protected void parseAttributeValue(Map<String, String> into) throws JETException
	{
		// Get the attribute name:
		skipSpaces();
		String name = parseToken(false);

		if (!JET_ATTRIBUTES.contains(name))
		{
			throw new JETException(name + " " + mark().toDecoratedShortString());
		}
	...
	/**
	 * Parse some tag attributes for Beans. The stream is assumed to be positioned right after the tag name. The syntax
	 * recognized is:
	 *
	 * <pre>
	 ...
	 *
	 * @return A HashMap mapping String instances (variable names) into String instances (variable values).
	 */
	public Map<String, String> parseTagAttributesBean() throws JETException
	{
		Map<String, String> values = new HashMap<>(11);
		while (true)
		{
			skipSpaces();
			int ch = peekChar();
		...
		// Reached EOF:
		//
		throw new JETException(CodeGenPlugin.getPlugin().getString("jet.error.tag.attr.unterminated", //$NON-NLS-1$
			new Object[] {mark().toString()}));
	}

	/**
	 * Parse some tag attributes. The stream is assumed to be positioned right after the tag name. The syntax recognized
	 * is:
	 *
	 * <pre>
	 ...
	 * @return A HashMap mapping String instances (variable names) into String instances (variable values).
	 */
	public Map<String, String> parseTagAttributes() throws JETException
	{
		Map<String, String> values = new HashMap<>(11);
		while (true)
		{
	...
	/**
	 * Parse utilities - Is current character a token delimiter ? Delimiters are currently defined to be =, &gt;, &lt;,
