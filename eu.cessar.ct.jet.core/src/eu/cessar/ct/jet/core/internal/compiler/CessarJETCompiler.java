 ...
 *
 * File created by uidu0944 May 24, 2012 1:45:10 PM </copyright>
 */
...
public class CessarJETCompiler implements CessarJETParseEventListener
{
	...
	protected CessarJETParser parser;
	...
	protected List<JETConstantDataGenerator2> constants = new ArrayList<JETConstantDataGenerator2>(100);

	protected Map<char[], JETConstantDataGenerator2> constantDictionary = new HashMap<char[], JETConstantDataGenerator2>(
		100, 100);
	...
	/**
	 * If fNoNewLineForScriptlets is true, the trailing newline/space sequence is stripped from each character data
	 * segment, and stored in this field. Depending on what follows, it may then be discarded or handled as its own
	 * character data segment.
	 */
	protected char[] fSavedLine = null;

	/**
	 * The depth of the current section, where 0 is outside of any sections. A section is delimited by start and end
	 * directives, and must be preceded by an include directive with fail="alternative".
	 */
	protected int sectionDepth = 0;

	/**
	 * Whether content is currently being skipped. This is set according to skipSections, as sections are started and
	 * ended.
	 */
	...
	public CessarJETCompiler(String templateURI) throws JETException
	{
		this(templateURI, "UTF8");
	}

	public CessarJETCompiler(String templateURI, String encoding) throws JETException
	{
		this(templateURI, openStream(templateURI), encoding);
	}

	public CessarJETCompiler(String templateURI, InputStream inputStream, String encoding) throws JETException
	{
		super();

		this.templateURI = templateURI;
		reader = new CessarJETReader(templateURI, inputStream, encoding);
	}

	public CessarJETCompiler(String[] templateURIPath, String relativeTemplateURI) throws JETException
	{
		this(templateURIPath, relativeTemplateURI, "UTF8");
	}

	public CessarJETCompiler(String[] templateURIPath, String relativeTemplateURI, String encoding) throws JETException
	{
		super();

		this.templateURIPath = templateURIPath;
		templateURI = relativeTemplateURI;
		String[] actualTemplateURI = findLocation(templateURIPath, 0, relativeTemplateURI);
		reader = new CessarJETReader(actualTemplateURI[1], relativeTemplateURI, openStream(actualTemplateURI[0]),
			encoding);
	}
	...
	public void handleDirective(String directive, CessarJETMark start, CessarJETMark stop,
		Map<String, String> attributes) throws JETException
	{
		if (directive.equals("include"))
		{
			...
			if (fileURI != null)
			{
				...
				try
				{
					...
					// The include succeeded, so if there is an alternative and
					// we're not skipping, we need to start.
					//
					...
					// If a newline from the previous character data remains,
					// leave it around to be processed as if it appeared in the
					// included file.
					//
					if (fSavedLine != null)
					{
				...
				{
					// The include failed, so if there is an alternative, we
					// don't skip it.
					//
		...
		else if (directive.equals("start"))
		{
			...
			// A section is not allowed without a preceding include with
			// alternative.
			//
		...
		else if (directive.equals("end"))
		{
			...
			// This pop is safe because a section couldn't have been started
			// without an include that pushed.
			//
			...
			else
			{
				...
				if (skeletonURI != null)
				{
					try
					{
						...
						skeleton.setCompilationUnitContents(skeletonEncoding == null ? new String(input) : new String(
							input, skeletonEncoding));
					}
	...
	public void handleExpression(CessarJETMark start, CessarJETMark stop, Map<String, String> attributes)
		throws JETException
	{
		if (skipping)
		{
		...
		JETGenerator gen = new JETExpressionGenerator2(reader.getChars(start, stop));
		addGenerator(gen);
	}

	public void handleScriptlet(CessarJETMark start, CessarJETMark stop, Map<String, String> attributes)
		throws JETException
	{
		if (skipping)
		{
		...
		JETGenerator gen = new JETScriptletGenerator2(reader.getChars(start, stop));
		addGenerator(gen);
	}
	...
	{
		// If a newline from the previous character data remains, add a
		// generator for it.
		//
		if (fSavedLine != null)
		{
	...
	{
		// An expression with more that 931 "+" will break Sun and IBM javac
		// compilers.
		//
		if (chars.length > 500)
		{
	...
	public void endPageProcessing() throws JETException
	{
		...
		for (JETConstantDataGenerator2 jetConstantDataGenerator: constants)
		{
	...
	public void parse() throws JETException
	{
		...
		//
		CessarJETParser.Directive directive = new CessarJETParser.Directive();
		...
		CessarJETCoreElement[] coreElements = {directive, new CessarJETParser.QuoteEscape(),
			new CessarJETParser.Expression(), new CessarJETParser.Scriptlet()};

		Class<?>[] accept = {CessarJETParser.Directive.class, CessarJETParser.QuoteEscape.class,
			CessarJETParser.Expression.class, CessarJETParser.Scriptlet.class};

		parse(coreElements, accept);
	}

	protected void parse(CessarJETCoreElement[] coreElements, Class<?>[] accept) throws JETException
	{
		parser = new CessarJETParser(reader, this, coreElements);
		beginPageProcessing();
	...
	protected static String[] resolveLocation(String[] templateURIPath, int start, String baseLocationURI,
		String locationURI)
	{
