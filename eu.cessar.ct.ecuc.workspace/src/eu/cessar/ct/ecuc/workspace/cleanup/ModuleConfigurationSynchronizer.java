 ...
 * 
 * File created by aurel_avramescu Aug 16, 2010 5:52:54 PM </copyright>
 */
 ...
 * 
 */
public class ModuleConfigurationSynchronizer
{
	...
	private IEcucMMService ecucMMService;
	 ...
	 * @param config
	 */
	public ModuleConfigurationSynchronizer(IProject project, GModuleConfiguration config)
	{
		ecucModel = IEcucCore.INSTANCE.getEcucModel(project);
		mmService = MMSRegistry.INSTANCE.getMMService(project);
		ecucMMService = MMSRegistry.INSTANCE.getMMService(project).getEcucMMService();
		this.config = config;
	}
	...
	public List<GModuleConfiguration> performSync(GModuleConfiguration config)
	{
		...
		for (GContainerDef containerDef: def.gGetContainers())
		{
			...
			if (goToNextLevel)
			{
				...
				// sync lower multiplicity
			}
			syncContainerMultiplicity(splitedModulesConf.get(0), containerDef, splitedContainers);
		}
		...
		while (iterator.hasNext())
		{
			...
			if (eSplitableList.size() > 0)
			{
				...
				for (GContainerDef subContainerDef: subcontainersDefs)
				{
					...
					syncContainerMultiplicity(eSplitableList.get(0), subContainerDef,
						splitedSubContainers);
				}
			}
		}
	}
	...
	private void performParametersSync(List<GContainer> owners, GContainerDef containerDef)
	{
		...
		for (GConfigParameter parameterDef: ((GParamConfContainerDef) containerDef).gGetParameters())
		{
			...
			try
			{
				if (ecucMMService.isSetDefaultValue(parameterDef))
				{
					BigInteger lowerMultiplicity = MultiplicityUtils.getLowerMultiplicity(parameterDef);
					int dif = lowerMultiplicity.intValue() - number;
					if (dif > 0)
					{
						IEcucCleaningAction action = new EcucCleaningActionImpl(
							MetaModelUtils.getAbsoluteQualifiedName(containerDef),
							ECleaningProblemsType.LOWER_MULTIPLICITY_IS_BIGGER,
							Messages.ModuleConfigurationCleaner_Parameter,
							new SyncActionCreateParameter(owners.get(0), parameterDef, dif));
						cleaningActions.add(action);
					}
				}
			}
	...
	private void performRefValuesSync(List<GContainer> owners, GContainerDef containerDef)
	{
		...
		for (GConfigReference refValueDef: ((GParamConfContainerDef) containerDef).gGetReferences())
		{
			...
			/*BigInteger lowerMultiplicity = MultiplicityUtils.getLowerMultiplicity(refValueDef);
			int dif = lowerMultiplicity.intValue() - number;
			if (dif > 0)
			{
				IEcucCleaningAction action = new EcucCleaningActionImpl(
					MetaModelUtils.getAbsoluteQualifiedName(containerDef),
					ECleaningProblemsType.LOWER_MULTIPLICITY_IS_BIGGER,
					Messages.ModuleConfigurationCleaner_Reference, new SyncActionCreateReference(
						owners.get(0), refValueDef, dif));
				cleaningActions.add(action);
			}*/
		}
	}
	...
	private void syncContainerMultiplicity(GIdentifiable owner, GContainerDef containerDef,
		List<SplitedEntry<GContainer>> splitedContainers)
	{
		boolean instanceExists = splitedContainers != null && !splitedContainers.isEmpty();
		int nr = 0;
		if (instanceExists)
		{
			Collection<ESplitableList<GContainer>> splitedCollection = new ArrayList<ESplitableList<GContainer>>();
			...
			Iterator<ESplitableList<GContainer>> iterator = splitedCollection.iterator();
			List<String> existingShortNames = new ArrayList<String>(10);
			while (iterator.hasNext())
			{
				ESplitableList<GContainer> next = iterator.next();
				if (next.size() > 0)
				{
					if (!existingShortNames.contains(next.get(0).gGetShortName()))
					{
						nr++;
						existingShortNames.add(next.get(0).gGetShortName());
					}
				}
			}
		}

		BigInteger lowerMultiplicity = MultiplicityUtils.getLowerMultiplicity(containerDef);
		int dif = lowerMultiplicity.intValue() - nr;
		if (dif > 0)
		{
			IEcucCleaningAction action = new EcucCleaningActionImpl(
				MetaModelUtils.getAbsoluteQualifiedName(containerDef),
				ECleaningProblemsType.LOWER_MULTIPLICITY_IS_BIGGER,
				Messages.ModuleConfigurationCleaner_Container, new SyncActionCreateContainer(owner,
					containerDef, dif));
			cleaningActions.add(action);
		}
