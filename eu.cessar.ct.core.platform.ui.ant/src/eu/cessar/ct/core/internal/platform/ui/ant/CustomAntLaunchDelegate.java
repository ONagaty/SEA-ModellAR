...
/**
 * Launch delegate for CESSAR-CT Ant files.
 * 
 */
@SuppressWarnings("restriction")
public class CustomAntLaunchDelegate extends AntLaunchDelegate
{
	...
	/**
	 * String attribute identifying the build scope for a launch configuration. <code>null</code> indicates the default
	 * workspace build.
	 * 
	 * Note: this attribute was used with the old 'AntBuildTab' which has been replaced by the 'ExternalToolsBuildTab'.
	 * The 'ExternalToolsBuildTab' uses a different attribute key, so use the external tools attribute when present:
	...
	/**
	 * Attribute identifier specifying whether referenced projects should be considered when computing the projects to
	 * build. Default value is <code>true</code>.
	 * 
	 * Note: this attribute was used with the old 'AntBuildTab' which has been replaced by the 'ExternalToolsBuildTab'.
	 * The 'ExternalToolsBuildTab' uses a different attribute key, so use the external tools attribute when present:
	 * IExternalToolConstants.ATTR_INCLUDE_REFERENCED_PROJECTS
	 */
	private static final String ATTR_INCLUDE_REFERENCED_PROJECTS = AntLaunching.getUniqueIdentifier()
	 ...
	 * 
	 * @see
	 * org.eclipse.ant.internal.ui.launchConfigurations.AntLaunchDelegate#launch(org.eclipse.debug.core.ILaunchConfiguration
		...
		throws CoreException
	{
		try
		{
			...
			{
				monitor.beginTask(MessageFormat.format(AntLaunchConfigurationMessages.AntLaunchDelegate_Running__0__2,
			...
			{
				IStatus status = new Status(IStatus.ERROR, AntLaunching.PLUGIN_ID, 1, MessageFormat.format(
					AntLaunchConfigurationMessages.AntLaunchDelegate_Build_In_Progress,
					new String[] {location.toOSString()}), null);
			...
			{// create a copy so as to not affect the
			...
			if (!isSeparateJRE)
			{
				runner = configureAntRunner(configuration, location, basedir, idProperty, arguments, userProperties,
					propertyFiles, targets, customClasspath, antHome, setInputHandler);
			}
			...
			StringBuffer commandLine = generateCommandLine(location, arguments, userProperties, propertyFiles, targets,
				antHome, basedir, isSeparateJRE, captureOutput, setInputHandler);
			...
			if (isSeparateJRE)
			{
				monitor.beginTask(MessageFormat.format(
					AntLaunchConfigurationMessages.AntLaunchDelegate_Launching__0__1,
					new String[] {configuration.getName()}), 10);
				runInSeparateVM(configuration, launch, monitor, idStamp, antHome, port, requestPort, commandLine,
					captureOutput, setInputHandler);
			}
			else
			{
	...
	private void runInSameVM(ILaunchConfiguration configuration, ILaunch launch, IProgressMonitor monitor,
		IPath location, String idStamp, AntRunner runner, StringBuffer commandLine) throws CoreException
	{
		...
		final CustomAntProcess process = new CustomAntProcess(location.toOSString(), launch, attributes);
			...
			Runnable r = new Runnable()
			{
				public void run()
				{
					...
					catch (CoreException e)
					{
						...
						exitStatus = new Status(IStatus.ERROR, CessarPluginActivator.PLUGIN_ID, 1, e.getMessage(), e);
	...
	private AntRunner configureAntRunner(ILaunchConfiguration configuration, IPath location, String baseDir,
		StringBuffer idProperty, String[] arguments, Map<String, String> userProperties, String[] propertyFiles,
		String[] targets, URL[] customClasspath, String antHome, boolean setInputHandler) throws CoreException
	{
	...
	private StringBuffer generateCommandLine(IPath location, String[] arguments, Map userProperties,
		String[] propertyFiles, String[] targets, String antHome, String basedir, boolean separateVM,
		boolean captureOutput, boolean setInputHandler)
	{
		...
		// really only Eclipse generated properties
	...
	private static void appendTaskAndTypes(AntCorePreferences prefs, StringBuffer commandLine)
	{
	...
	private static void appendProperty(StringBuffer commandLine, String name, String value)
	{
	...
	private void runInSeparateVM(ILaunchConfiguration configuration, ILaunch launch, IProgressMonitor monitor,
		String idStamp, String antHome, int port, int requestPort, StringBuffer commandLine, boolean captureOutput,
		boolean setInputHandler) throws CoreException
	{
		...
		if (debug)
		{ // do not allow launch in foreground bug 83254
	...
	private void setDefaultVM(ILaunchConfiguration configuration, ILaunchConfigurationWorkingCopy copy)
	{
		try
		{
			...
			// Java launching
			// concept of default VM
			// copy.setAttribute(IJavaLaunchConfigurationConstants.ATTR_VM_INSTALL_NAME,
			// (String) null);
			// copy.setAttribute(IJavaLaunchConfigurationConstants.ATTR_VM_INSTALL_TYPE,
			// (String) null);
		...
		catch (CoreException ce)
		{
			...
			copy.setAttribute(IJavaLaunchConfigurationConstants.ATTR_JRE_CONTAINER_PATH,
				JavaRuntime.getDefaultJREContainerEntry().getPath().toString());
			// copy.setAttribute(IJavaLaunchConfigurationConstants.ATTR_VM_INSTALL_NAME,
	...
	/**
	 * Sets the default working directory to be the parent folder of the buildfile if the user has not explicitly set
