 ...
 * 
 */
public abstract class GConfigReferenceDefPMInfo extends AbstractParentedPMInfo<GConfigReference>
{
	...
	private void createParentReference(IEcucPresentationModel ecucPM)
	{
		if (classifier != null)
		{
			...
			reference.setName(EMFPMUtils.genName(getParentClass().getEAllStructuralFeatures(),
				element.gGetShortName()));
			reference.setContainment(false);
			reference.setChangeable(true);
			reference.setUnique(false);
			reference.setUnsettable(true);
			reference.setLowerBound(ecucMMService.getLowerMultiplicity(element, BigInteger.ZERO,
				true).intValue());
			...
			{
				reference.setUpperBound(value);
			}
			EMFPMUtils.setProxyAnnotation(reference, "TYPE", element.eClass().getName());
			EMFPMUtils.setProxyAnnotation(reference, "NAME", element.gGetShortName());
			getParentClass().getEStructuralFeatures().add(reference);
		}
	}
