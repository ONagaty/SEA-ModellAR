...
public class CessarOldLicenseHandler
{
	...
	/**
	 * Return the message to display in the splash screen with the information on the license associated to the given
	 * product
	 *
	 * @param productName
	 *        the product name
	 ...
	 * @param moduleName
	 *        the module name
	 * @return true if the license is valid for the given module of the given product, otherwise false
	 */
	...
	public static void _setLicenseFolderIfNeeded(String productName)
	{
		...
		if (licenseHandler.getLicenseFolder() == null)
		{
			...
			if (cessarLicFolderStr != null)
			{
				...
				/*
				 * File cessarLicFolder = new File(cessarLicFolderStr); // Checks if a license exists in the folder if
				 * (licenseHandler.checkIfLicenseExistInFolder(cessarLicFolder, productName)) { licFolder =
				 * cessarLicFolder; }
				 */
			}
			else
