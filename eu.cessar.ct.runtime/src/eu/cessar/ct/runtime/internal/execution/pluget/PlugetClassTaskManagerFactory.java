 ...
 * 
 */
public class PlugetClassTaskManagerFactory extends AbstractTaskManagerFactory<String>
{
	/* (non-Javadoc)
	...
	public ICessarTaskManagerImpl<String> createManager(IProject project, Map<String, Object> map)
	{
		...
		{// need to locate the compatibility mode task manager
			...
			if (mmService != null)
			{
				...
				{
					String taskName = compatibilityService.getPlugetClassTaskTypeName();
					return (ICessarTaskManagerImpl<String>) ExecutionService.createManager(project,
						taskName, map);
				}
			}
		}
		PlugetClassTaskManager manager = new PlugetClassTaskManager(getDescriptor(), project);
		return manager;
	}
