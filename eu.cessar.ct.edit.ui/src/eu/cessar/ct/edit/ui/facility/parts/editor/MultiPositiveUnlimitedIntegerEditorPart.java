...
import eu.cessar.ct.core.platform.ui.widgets.MultiPositiveBigIntegerEditor;
 ...
 *
 */
public class MultiPositiveUnlimitedIntegerEditorPart extends AbstractMultiNumericDatatypeEditorPart<BigInteger>
implements IIntegerEditorPart
{
	private ERadix radix;
	private MultiPositiveBigIntegerEditor editor;

	/**
	 * @param editor
	 */
	public MultiPositiveUnlimitedIntegerEditorPart(IModelFragmentFeatureEditor editor)
	{
		super(editor);
	}
	...
	protected IDatatypeEditor<List<BigInteger>> createDatatypeEditor()
	{
		...
		MultiDatatypeValueHandler<BigInteger> handler = new MultiDatatypeValueHandler<BigInteger>(handlerLabelProvider,
			getInputFeature().getName(), getInputFeature().getName());
		editor = new MultiPositiveBigIntegerEditor(handler, getRadix());
