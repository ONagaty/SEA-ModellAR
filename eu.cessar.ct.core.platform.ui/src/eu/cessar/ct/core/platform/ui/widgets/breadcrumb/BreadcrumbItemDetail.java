 ...
 */
public class BreadcrumbItemDetail
{

	private final BreadcrumbItem parentItem;

	protected final Composite contentComposite;
	protected final Composite imageComposite;
	protected final Composite textComposite;
	...
	{
		return contentComposite.isFocusControl() || imageComposite.isFocusControl()
			|| textComposite.isFocusControl() || imageElement.isFocusControl()
		...
		composite.addListener(SWT.KeyDown, new Listener()
		{
			public void handleEvent(final Event event)
			{
				...
				switch (event.keyCode)
				{
					...
					case SWT.ARROW_DOWN:
					case SWT.KEYPAD_ADD:
						if (parentItem != viewer.getWorkspaceItem())
						{
							if (!selected)
							{
								viewer.selectItem(parentItem);
							}
							if (!viewer.isDropDownOpen())
							{
								openDropDown();
							}
							else
							{
								Shell shell = viewer.getDropDownWindow().getShell();
						...
						{
							if (!viewer.getWorkspaceItem().isMenuShown())
							{
								openDropDown();
							}
							else
			...
			private void openDropDown()
			{
				...
				if (index > -1)
				{
					parent = viewer.getItem(Math.max(0, index));
				}
				else
				{
					parent = viewer.getWorkspaceItem();
				}
	...
	protected void enableRightClick(boolean isRightClick)
	{
